...
- `scripts/make-signing-cert.sh` — one-off: create the stable local signing identity (see Footguns)
...
- Keychain "Always Allow" is tied to the binary's code signature — every ad-hoc rebuild re-prompts. `scripts/make-signing-cert.sh` once stops it; `bundle.sh` picks the identity up automatically.
