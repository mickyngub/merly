...
- **Never change `CFBundleIdentifier`.** The notification grant and the Keychain grant both hang off it, so a rename silently throws them away — and the permission prompt does not reliably reappear for an id macOS has already refused. Cost a long debugging session; see [docs/specs/distribution.md](docs/specs/distribution.md) § Notifications.
