...
Merly reads authentication tokens belonging to other tools on your machine, so
...
Merly is a read-only usage viewer. It never signs you in, never creates
...
|---|---|---|
| Claude | macOS Keychain item `Claude Code-credentials[-<hash>]`, read via `/usr/bin/security` | The OAuth token created by Claude Code itself. Merly never refreshes it. |
| Codex | `~/.codex/auth.json` | The ChatGPT OAuth token created by the Codex CLI. Merly never refreshes it. |
| Kimi | `~/.kimi-code/credentials/kimi-code.json` | 15-minute tokens. Merly **must** refresh them, following the CLI's exact lockfile protocol, and writes the rotated credentials back to that same file (see below). |
...
`~/Library/Application Support/Merly/` (`providers.json`, `usage-cache.json`,
`last-good.json`) — plus, for Kimi only, the rotated credential file named above.

## Where data goes on the network

Merly makes outbound HTTPS requests to these hosts and no others:
...
logged, **never** written to disk by Merly (the one exception being Kimi's own
...
- **Claude & Codex tokens are never refreshed.** If a token is expired, Merly
...
- **Kimi token refresh is done safely.** Because Kimi's refresh token rotates
  and is single-use, Merly acquires the CLI's own `flock` before refreshing,
  re-reads under the lock, and atomically persists the new credentials — so it
  cannot invalidate your `kimi` CLI login.
- Merly has no telemetry, analytics, or crash reporting. It phones home to no
  one.

## Distribution & code signing

Merly is distributed **as source only** — there is no signed, notarized build and
...
**Treat any `Merly.dmg` or prebuilt `Merly.app` offered to you as untrusted** — we
...
1. Go to the repository's **Security** tab → **Report a vulnerability**
   (`https://github.com/mickyngub/merly/security/advisories/new`).
...
Merly is source-distributed. Version tags (`v*`) exist so a built app can
